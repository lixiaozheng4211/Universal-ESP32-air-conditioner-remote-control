...
int FanSpeeds =
    0; // 设置风速默认值，0自动，1为20%，2为40%，3为60%，4为80%，5为100%
...
void Send_Byte(
    uint8_t data1, int nbit1,
    bool
        msb); // 定义发送原始数据的函数，data1(数据），nbit1(数据二进制位数），msb(数据发送顺序，1为低位先发，0为高位先发）

void Send_Code_S(uint8_t A, uint8_t B, uint8_t C, uint8_t D_1, uint8_t D_2,
                 uint8_t D_3, uint8_t D_4, uint8_t D_5, uint8_t D_6);
// 定时用的数据发送函数，因为定时发送的数据C码的反码位置为固定的11111111，所以引入该函数，C的反码位置为固定的0xFF

void Send_Code_L(uint8_t A, uint8_t B, uint8_t C, uint8_t D_1, uint8_t D_2,
                 uint8_t D_3, uint8_t D_4, uint8_t D_5, uint8_t D_6);
// 正常的发送数据的函数，用来发送长码

void if_D6(
    int fs); // 计算风速数据的函数，因为风速数据的D_6和温度是否有0.5位相关联。

void Send_Meidi(
    bool
        Code_State); // 发送长码数据的函数，1为正常发送，0为C反码固定为0xFF的发送。
...
void IRsendMeidi::setCodeTime(int marks, int one_spaces, int zero_spaces,
                              int l_marks, int l_spaces, int s_spaces) {
  Marks = marks;
...
void IRsendMeidi::Send_Code(uint8_t AC, uint8_t BC,
                            uint8_t CC) { // 发送ABC码的函数
...
void Send_Code_S(uint8_t AC, uint8_t BC, uint8_t CC, uint8_t D1C, uint8_t D2C,
                 uint8_t D3C, uint8_t D4C, uint8_t D5C, uint8_t D6C) {
...
void Send_Code_L(uint8_t AC, uint8_t BC, uint8_t CC, uint8_t D1C, uint8_t D2C,
                 uint8_t D3C, uint8_t D4C, uint8_t D5C, uint8_t D6C) {
