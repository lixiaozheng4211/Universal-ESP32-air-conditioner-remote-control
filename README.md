...
主界面包含串口连接区、开启空调、关闭所有空调、添加空调、删除空调、详情控制、38K 测试、40K 测试。已保存空调以卡片网格展示，可以直接看到名称、开关状态、温度、模式、风速、上下风、左右风和遥控器信息。
...
6. “开启空调”和“关闭所有空调”会遍历本地空调库并按约 `1500ms` 间隔逐台发送 `action=power` 命令，避免红外命令挤在一起。
7. 断开 USB 线测试心跳保护：Qt 应弹出断线提示，重插后点击“重新连接”恢复串口通信。
8. 38K/40K 测试按钮会发送 `IRTEST`，用于检查 GPIO4、红外 LED、载波频率和接收头是否能看到信号。
